...
function stopCronJobs() {
  ...
  if (_cronTasks._spotMonitorInterval) clearInterval(_cronTasks._spotMonitorInterval);
  _cronTasks = [];
}
...
Summarize the current portfolio health, total fees earned, and performance of all open positions. Recommend any high-level adjustments if needed.
  ...
  // Throttles the equity sample taken while the portfolio is flat (no open positions).
  let _lastFlatEquityAt = 0;
  let _pnlPollBusy = false;
  const pnlPollInterval = setInterval(async () => {
    if (_managementBusy || _screeningBusy || _pnlPollBusy) return;
    _pnlPollBusy = true;
    try {
      const hasTracked = getTrackedPositions(true).length > 0;
      const result = hasTracked
        ? await getMyPositions({ force: true, silent: true }).catch(() => null)
        : null;

      // Feed the portfolio breaker a FULL equity reading (wallet + open positions).
      // Wallet balance alone would hide a position bleeding out that has not closed,
      // which is exactly what the drawdown limit exists to catch.
      //
      // This runs even with ZERO open positions. An earlier version returned before
      // this point when flat, so the equity series went blind exactly after a losing
      // streak had closed everything out — the moment the drawdown limit matters most.
      // Sampling is throttled because a flat wallet does not need a 3s cadence.
      const nowMs = Date.now();
      if (hasTracked || nowMs - _lastFlatEquityAt > 60_000) {
  ...
  // Spot monitor — deterministic exit rules for the spot venue, no LLM. Runs only when
  // the venue is enabled; a disabled venue must cost nothing.
  let spotMonitorInterval = null;
  if (config.venue?.spot) {
    ...
    spotMonitorInterval = setInterval(async () => {
      ...
      _spotBusy = true;
      try {
        const r = await monitorSpot();
        for (const a of r.actions || []) {
          log("spot", `${a.symbol}: ${a.reason} -> ${a.result?.success ? `closed ${a.result.pnl_pct}%` : `FAILED ${a.result?.error}`}`);
        }
      } catch (e) {
        log("spot_error", `Spot monitor failed: ${e.message}`);
      } finally {
        _spotBusy = false;
      }
    }, spotMs);
    log("cron", `Spot monitor every ${config.spot.monitorIntervalSec}s`);
  }
  ...
  if (config.opportunity.enabled) {
    ...
    opportunityPollInterval = setInterval(async () => {
      ...
      // Don't burn LLM tokens hunting for entries we are not allowed to take.
      if (!riskGuard.canDeploy().pass) return;
  ...
  _cronTasks._spotMonitorInterval = spotMonitorInterval;
...
Focus on: hold duration, entry/exit timing, what win rates look like, whether scalpers or holders dominate.`,
    ...
    if (input === "/evolve") {
      await runBusy(async () => {
        ...
        const result = evolveThresholdsBidirectional(lessonsData.performance, config);
