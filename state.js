...
export function syncOpenPositions(active_addresses) {
  ...
  for (const posId in state.positions) {
    ...
    // This path bypasses recordPerformance entirely, so the realized PnL of a position
    // closed outside the agent (manually in the Meteora UI, or a close whose bookkeeping
    // we lost) never reaches the risk ledger. The PnL is genuinely unrecoverable here —
    // the position is already gone from chain — so record the HOLE rather than invent a
    // number. Without this the daily-loss and drawdown limits silently understate.
    import("./risk.js")
      .then(({ riskGuard }) => riskGuard.recordUnaccountedClose({
        position: posId,
        note: pos.pool_name || pos.pool || null,
      }))
      .catch((error) => log("risk_warn", `Could not record unaccounted close: ${error.message}`));
  }
