...
import * as _llmProviders from "./llm-providers.js";
...
// Any OpenAI-compatible endpoint: OpenRouter, Anthropic, OpenAI, Groq, DeepSeek,
// Together, xAI, Gemini, or a local LM Studio / Ollama / vLLM server. See
// llm-providers.js for the registry and how the base URL and key are resolved.
//
// Constructed lazily and re-resolved on every call. Meridian built the client at module
// load from a hard-coded OpenRouter URL, so importing agent.js without a key threw (a
// fresh install could not boot far enough to reach the panel where you enter that key),
// and a provider or key changed through the panel needed a full restart to take effect.
let _client = null;
function getClient() {
  const { resolveLlm } = _llmProviders;
  const r = resolveLlm(readUserConfigForLlm());

  if (!r.ok) {
    throw new Error(
      `LLM not configured (${r.label}): ${r.problems.join(" ")} ` +
      `Set it in the control panel under Settings, or in .env.`,
    );
  }

  if (!_client || _client._hmKey !== r.apiKey || _client._hmBase !== r.baseUrl) {
    _client = new OpenAI({ baseURL: r.baseUrl, apiKey: r.apiKey, timeout: 5 * 60 * 1000 });
    _client._hmKey = r.apiKey;
    _client._hmBase = r.baseUrl;
    log("agent", `LLM provider: ${r.label} (${r.baseUrl})`);
  }
  return _client;
}

/**
 * Re-read user-config.json for LLM fields only. Cheap (a few hundred bytes) and only
 * on the provider-resolution path, but it is what lets a key saved in the panel work
 * without a restart.
 */
function readUserConfigForLlm() {
  try {
...
/**
 * Fallback model when a role has none configured. Provider-aware: a hard-coded
 * "openrouter/healer-alpha" is a guaranteed 404 on Anthropic, Groq or a local server,
 * and Meridian used exactly that string regardless of endpoint.
 */
function defaultModelFor() {
  if (process.env.LLM_MODEL) return process.env.LLM_MODEL;
  try {
    const r = _llmProviders.resolveLlm(readUserConfigForLlm());
    return r.exampleModel || "openrouter/healer-alpha";
  } catch {
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  if (agentType === "SCREENER") {
    try {
      ...
      // Our own generalised playbook — trusted, unlike swarm evidence, because it is
      // derived from positions this wallet actually closed.
      const { getPlaybookForPrompt } = await import("./self-learning.js");
      const playbook = getPlaybookForPrompt();
      if (playbook) weightsSummary = weightsSummary ? `${playbook}\n\n${weightsSummary}` : playbook;
    } catch { /* signal-weights not critical */ }
  }
  // Swarm evidence is fetched separately from our own lessons and rendered in its own
  // fenced, untrusted section — never merged into LESSONS LEARNED.
  let swarmBlock = null;
  try {
    const { getSwarmBlock } = await import("./lessons.js");
    swarmBlock = getSwarmBlock(agentType);
  } catch { /* swarm input is optional; never block a cycle on it */ }

  const systemPrompt = buildSystemPrompt(agentType, portfolio, positions, stateSummary, lessons, perfSummary, weightsSummary, decisionSummary, swarmBlock);
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      const activeModel = model || defaultModelFor();

      // Retry up to 3 times on transient provider errors (502, 503, 529)
      // Only meaningful on OpenRouter — that model id does not exist anywhere else, so
      // "falling back" to it on another provider turns a transient 502 into a hard 404.
      const FALLBACK_MODEL = _llmProviders.resolveLlm(readUserConfigForLlm()).provider === "openrouter"
