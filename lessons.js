...
export async function recordPerformance(perf) {
  ...
  // Feed the portfolio breaker BEFORE anything that can throw. A close that is not
  // counted is a close the risk limits never see, so this must not sit behind lesson
  // derivation, pool-memory writes or swarm pushes.
  try {
    const { recordClosedPosition } = await import("./risk.js");
    const trip = recordClosedPosition(entry, perf.sol_usd ?? null);
    if (trip?.halted) log("risk", `Breaker tripped after close: ${trip.reason}`);
  } catch (error) {
    log("risk_warn", `Could not record close with the risk breaker: ${error.message}`);
  }
  ...
  if (data.performance.length % MIN_EVOLVE_POSITIONS === 0) {
    ...
    // Bidirectional evolution replaces the original one-way ratchet, which could only
    // ever tighten and so could walk screening into a state where nothing qualified.
    const { evolveThresholdsBidirectional } = await import("./self-learning.js");
    const result = evolveThresholdsBidirectional(data.performance, config);
 ...
 */
/** @deprecated One-way ratchet — superseded by self-learning.js#evolveThresholdsBidirectional.
 *  Kept only so an operator can diff the old behaviour. Nothing calls it. */
...
export function getLessonsForPrompt(opts = {}) {
  ...
  // NOTE: swarm lessons are deliberately NOT merged in here. Meridian appended them as
  // a "── HIVEMIND ──" section inside this same block, which the prompt then labels
  // "LESSONS LEARNED" — i.e. text written by strangers landed in the trusted region of
  // the system prompt. Swarm data is now rendered separately by getSwarmBlock(), inside
  // its own nonce fence, and the prompt marks it as data.
  const sections = [];
...
/**
 * Swarm evidence, kept strictly separate from our own lessons.
 * Returns a self-fencing block or null.
 */
export function getSwarmBlock(agentType = "GENERAL") {
  return getSharedLessonsForPrompt({ agentType });
}
