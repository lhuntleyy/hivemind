...
async function runSafetyChecks(name, args) {
  switch (name) {
    case "deploy_position": {
      ...
      // Checked FIRST and before any network call: once tripped, nothing about this
      // pool can make opening new risk correct. Only OPENING is gated — close, claim
      // and swap stay available, because a breaker that blocks the exit is worse than
      // no breaker at all.
      const risk = riskGuard.canDeploy();
      if (!risk.pass) return risk;
