...
import { guardZapIn } from "../hive/solana-inspector.js";
import { noteDeploy } from "../self-learning.js";
...
export async function deployPosition({
  ...
  if (shouldUseLpAgentRelayForDeploy()) {
    try {
      ...
      // ── Relay guard ────────────────────────────────────────────
      // The relay BUILDS this transaction; we only sign it. Meridian simulated and
      // capped SOL loss on the close path but signed the DEPLOY path blind — the one
      // direction that actually sends SOL out. Same guard, both directions now.
      await guardZapIn([...addLiquidityUnsigned, ...swapUnsigned], {
        connection: getConnection(),
        owner: wallet.publicKey,
        amountSol: finalAmountY,
        pool: pool_address,
        overheadSol: config.risk?.relayOverheadSol ?? 0.05,
      });
      ...
      noteDeploy();   // resets the starvation counter used by bidirectional threshold evolution
