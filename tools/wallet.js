...
// Slippage bounds for every swap this agent makes.
//   floor   — below ~0.5% a thin memecoin swap often fails to route at all, and an
//             exit that never executes is worse than a costly one.
//   ceiling — above 5% a fill stops being a price and becomes a donation. For
//             comparison, the relay zap-out path ships 5000 bps (50%).
...
export async function swapToken({
  ...
  // SLIPPAGE. Meridian sent no slippage parameter at all, so every swap ran on
  // Jupiter's auto default with no ceiling the operator could see or set. On a thin
  // memecoin pair — which is every pair this agent trades — that is an open invitation
  // to be sandwiched on the way out, at exactly the moment a stop loss fires.
  const configured = Number(config.management?.swapSlippageBps);
  const requested = Number(slippage_bps);
  const bps = Math.min(
    MAX_SWAP_SLIPPAGE_BPS,
    Math.max(
      MIN_SWAP_SLIPPAGE_BPS,
      Number.isFinite(requested) ? requested : (Number.isFinite(configured) ? configured : DEFAULT_SWAP_SLIPPAGE_BPS),
    ),
  );
  ...
  if (process.env.DRY_RUN === "true") {
    return {
      dry_run: true,
      would_swap: { input_mint, output_mint, amount, slippage_bps: bps },
